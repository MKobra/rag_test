...
5. `HuggingFaceEndpointEmbeddings` вызывает Hugging Face Inference API, модель `paraphrase-multilingual-MiniLM-L12-v2` возвращает 384-мерный вектор для каждого чанка.
...
2. Вопрос кодируется той же моделью через Inference API.
